...
function Users() {
  ...
  const clubs = Array.from(
    new Set(users.map((u) => (u.club || "").trim()).filter(Boolean))
  );
  ...
  const filteredUsers = users.filter((u) => {
    ...
    if (
      filterSport &&
      (!u.deporte ||
        u.deporte.toLowerCase().trim() !== filterSport.toLowerCase().trim())
    )
      return false;
    if (filterPosition && filterPosition !== "") {
      if (availablePositions.length === 0) {
        if (
          !u.posicion ||
          u.posicion.toLowerCase().trim() !==
            filterPosition.toLowerCase().trim()
        )
          return false;
      } else {
        if (
    ...
    if (filterClub && filterClub !== "" && (!u.club || u.club !== filterClub))
      return false;
    return true;
  });
  ...
  return (
    <S.Section>
      ...
      <S.FilterRow>
        <S.FilterGroup>
          ...
          >
            ...
            {Array.from(
              new Set(users.map((u) => u.deporte).filter(Boolean))
            ).map((d) => (
        ...
        <S.FilterGroup>
          ...
          >
            ...
            {(availablePositions.length
              ? availablePositions
              : Array.from(
                  new Set(users.map((u) => u.posicion).filter(Boolean))
                )
            ).map((p) => (
      ...
      <S.UserCardsGrid>
        {filteredUsers.map((user) => (
          <S.UserCard key={user._id}>
            ...
            <S.UserInfo>
              <strong>Deporte:</strong> {user.deporte || "N/A"}
            </S.UserInfo>
            <S.UserInfo>
              <strong>Club:</strong> {user.club || "N/A"}
            </S.UserInfo>
            ...
            <S.CardActions>
              ...
              <S.ActionButton
                onClick={() => setSelectedTrainingsUser(user)}
                variant="success"
              >
                Ver entrenamientos
              </S.ActionButton>
        ...
        <UM.Overlay onClick={() => setSelectedUser(null)}>
          <UM.Modal onClick={(e) => e.stopPropagation()}>
            ...
            <UM.Header>
              <UM.AvatarBox>
                <UM.Avatar>
                  {selectedUser.nombre ? selectedUser.nombre.charAt(0) : "U"}
                  {selectedUser.apellido ? selectedUser.apellido.charAt(0) : ""}
                </UM.Avatar>
                <UM.Club>{selectedUser.club}</UM.Club>
                <UM.Club>
                  {selectedUser.deporte} · {selectedUser.posicion || "—"}
                </UM.Club>
              </UM.AvatarBox>
              ...
              <UM.MainInfo>
                ...
                <UM.Grid>
                  ...
                  <UM.Card>
                    ...
                    <UM.Value>
                      {selectedUser.altura ? `${selectedUser.altura} cm` : "—"}
                    </UM.Value>
                  </UM.Card>
                  <UM.Card>
                    <UM.Field>Peso</UM.Field>
                    <UM.Value>
                      {selectedUser.peso ? `${selectedUser.peso} kg` : "—"}
                    </UM.Value>
                  </UM.Card>
                  <UM.Card>
                  ...
                  <UM.Card>
                    ...
                    <UM.Value>
                      {selectedUser.nacimiento
                        ? selectedUser.nacimiento.split
                          ? selectedUser.nacimiento.split("T")[0]
                          : new Date(selectedUser.nacimiento)
                              .toISOString()
                              .split("T")[0]
                ...
                <UM.DetailsGrid>
                  ...
                  <UM.DetailsItem>
                    ...
                    {selectedUser.lesiones || "Ninguna"}
                  </UM.DetailsItem>
                  <UM.DetailsItem>
                    <strong>Zonas sensibles:</strong>{" "}
                    {selectedUser.zonasSensibles || "—"}
                  </UM.DetailsItem>
                  <UM.DetailsItem>
                    <strong>Días gym:</strong>{" "}
                    {selectedUser.diasDisponiblesGimnasio || "—"}
                  </UM.DetailsItem>
                  <UM.DetailsItem>
                    <strong>Días deporte:</strong>{" "}
                    {selectedUser.diasPracticaDeporte || "—"}
                  </UM.DetailsItem>
                  <UM.DetailsItem>
                    <strong>Próx. competencia:</strong>{" "}
                    {selectedUser.frecuenciaDeCompetencia
                      ? selectedUser.frecuenciaDeCompetencia.split
                        ? selectedUser.frecuenciaDeCompetencia.split("T")[0]
                        : new Date(selectedUser.frecuenciaDeCompetencia)
                            .toISOString()
                            .split("T")[0]
                      : "—"}
                  </UM.DetailsItem>
                  <UM.DetailsItem>
                    <strong>Salto (registro):</strong>{" "}
                    {selectedUser.salto
                      ? selectedUser.salto.split
                        ? selectedUser.salto.split("T")[0]
                        : new Date(selectedUser.salto)
                            .toISOString()
                            .split("T")[0]
                      : "—"}
                  </UM.DetailsItem>
                  <UM.DetailsItem>
                    <strong>Sprint (registro):</strong>{" "}
                    {selectedUser.sprint
                      ? selectedUser.sprint.split
                        ? selectedUser.sprint.split("T")[0]
                        : new Date(selectedUser.sprint)
                            .toISOString()
                            .split("T")[0]
                      : "—"}
                  </UM.DetailsItem>
      ...
      {selectedTrainingsUser && (
        <CoachUserTrainings
          userId={selectedTrainingsUser._id}
          user={selectedTrainingsUser}
          onClose={() => setSelectedTrainingsUser(null)}
        />
      )}
    </S.Section>
  );
